...
.PHONY: fix
fix:
	poetry run black -l 79 bindicator tests
	poetry run isort .

.PHONY: format
format:
	@echo "Running black" && poetry run black -l 79 --check bindicator tests || exit 1

.PHONY: lint
lint:
	@echo "Running flake8" && poetry run flake8 bindicator tests || exit 1
...
.PHONY: check
check:
	@echo "Running MyPy" && poetry run mypy bindicator || exit 1
...
.PHONY: run
run:
	poetry run python bindicator/main.py
